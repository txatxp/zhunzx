<template>
  <div class="z-article">
    <div class="z-article-box">
      <div class="z-article-left">
        <a
          :href="`${runtimeConfig.public.APP_URL}/allarticle?type=${item.type}&name=${item.name}`"
      ...
      <div class="z-article-right">
          <a :href="`${runtimeConfig.public.APP_URL}/allarticle?type=${type}&pageNum=${pageNum + 1}&name=${name}`" class="z-next-btn" v-if="article.length">换一批文章</a>
...
@media screen and (min-width: 1400px) {
  .z-article {
    ...
    position: relative;
    .z-article-box {
      width: 1400px;margin:auto;background-color: #fff;overflow:hidden;border-radius: 10px;padding: 30px;
    }
    .z-article-left {
      width: 300px;
      float: left;
